...
The relation is reflexive because all required diagonal pairs are present:
...
No pair of the form ((a,a)) is present.
...
The diagonal pairs are automatically symmetric because reversing them produces the same pair.
...
Combinations involving the diagonal pairs also satisfy transitivity.
...
A relation is any subset of (A\\times A).

A set with 25 elements has:
...
The remaining number of optional pairs is:

\[  
25-5=20  
\]

Each optional pair may be included or excluded.
...
There are five diagonal pairs, each of which may be included or excluded.
...
For every off-diagonal pair, either both ((a,b)) and ((b,a)) are included or both are excluded.

Therefore, the number of independent choices is:
...
\[  
H=\\text{students reading The Hindu}  
\]

\[  
T=\\text{students reading TOI}  
\]
...
### c) Tea and coffee but not milk

\[  
n(T\\cap C\\text{ only})=50-20  
\]
...
The 20 is added back because it was subtracted twice.
...
Let (M), (C) and (P) represent Mathematics, Computer Science and Physics.
